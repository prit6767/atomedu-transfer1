...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* legal pages */
  .legal { scroll-margin-top: 80px; }
  .legal-title { font-family: "Fraunces", Georgia, serif; font-size: clamp(40px, 6vw, 68px); letter-spacing: -0.03em; font-weight: 400; margin: 0 0 20px; line-height: 1.02; }
  .legal-title .it { font-style: italic; font-weight: 300; }
  .legal-lead { font-family: "Inter", sans-serif; font-size: 15.5px; color: var(--ink-soft); max-width: 65ch; margin: 0 0 36px; line-height: 1.65; }
  .legal-lead b { color: var(--ink); font-weight: 700; }
  .legal h3 { font-family: "Inter", sans-serif; font-size: 18px; font-weight: 700; margin: 30px 0 10px; color: var(--ink); letter-spacing: -0.005em; }
  .legal p { font-family: "Inter", sans-serif; font-size: 14.5px; line-height: 1.7; color: var(--ink-soft); max-width: 70ch; margin: 0 0 10px; }
  ...
  .ai-body {
    ...
    padding: 20px 22px; max-height: 55vh; overflow-y: auto;
  ...
  .ai-body:empty::before { content: "Waiting for Atom&hellip;"; color: var(--muted); font-family: "Inter", sans-serif; font-size: 13px; }
...
<section class="section legal" id="privacy">
  <div class="wrap">
    <h2 class="legal-title">Privacy <span class="it">policy.</span></h2>
    <p class="legal-lead"><b>Last updated: 2026.</b> Atom Edu is a free tool for teachers. This is the whole policy in plain English.</p>

    <h3>What we collect</h3>
    <p>Your name, work email, subject, and password (stored as-is in your own browser's localStorage, never leaves your device). Drafts you approve, students you add, and grades you enter are stored the same way, locally, on your device only. When you generate a worksheet, the input text and topic you provide are sent to Atom's inference backend to draft the worksheet. We do not persist those requests.</p>

    <h3>What we don't collect</h3>
    <p>No analytics. No cookies. No third-party trackers. No student names or grades ever leave your browser. We do not know who you are and we do not track individual accounts.</p>

    <h3>What we count (aggregate only)</h3>
    <p>Our admin dashboard tracks the total number of accounts and drafts made across the whole platform, as counts only, never as identifiable data. Emails, names, drafts, students, and grades are not visible to us.</p>

    <h3>Model calls</h3>
    <p>Every draft you generate goes through a large language model. The prompt you type and the class context you attached are the only things sent. Student work you upload for adaptation stays on your device and is not sent unless you explicitly ask Atom to work on it. When it is sent, it is sent for that one call and is not stored on our side and is not used to train any model, ours or a provider's.</p>

    <h3>Deletion</h3>
    <p>To delete everything, clear your browser storage for atom-edu.org. That is the whole delete flow, because there is nothing on our side to erase.</p>

    <h3>Children</h3>
    <p>Atom Edu is for teachers, not students. Students never sign in. We do not knowingly collect data from anyone under 13.</p>

    <h3>Changes</h3>
    <p>If this policy changes, the "Last updated" date at the top changes with it and the change is called out on the workspace home page the first time you visit after it happens.</p>

    <h3>Contact</h3>
    <p>Questions or concerns: hello@atom-edu.org.</p>
  </div>
</section>

<section class="section legal" id="terms">
  <div class="wrap">
    <h2 class="legal-title">Terms of <span class="it">use.</span></h2>
    <p class="legal-lead"><b>Last updated: 2026.</b> By using Atom Edu, you agree to the following. Also in plain English.</p>

    <h3>What Atom is</h3>
    <p>A free tool that drafts classroom materials for teachers. Atom drafts. You edit. You approve. Nothing goes out to a student, a family, or a gradebook until you say yes.</p>

    <h3>Who can use it</h3>
    <p>Teachers, instructional coaches, and other educators. You are responsible for what you approve and send. Atom is a drafting tool, not a source of truth.</p>

    <h3>Your account and content</h3>
    <p>Your account lives in your browser. The drafts, rosters, and notes you make are yours. We do not claim any ownership of what you create with Atom.</p>

    <h3>What you agree not to do</h3>
    <p>Do not use Atom to draft anything you would not send with your own name attached, and do not use it to harass, dox, or harm any student, family, colleague, or third party.</p>

    <h3>Cost</h3>
    <p>Free. Forever. Every feature. Every classroom. Every year you keep teaching. If that ever changes, existing teachers keep the free tier they signed up under.</p>

    <h3>No warranty</h3>
    <p>Atom drafts are a starting point. They can be wrong, out of date, or a bad fit for your class. Your judgment, your rubric, and your review are what makes an artifact ready. We offer Atom as-is, without warranty of any kind.</p>

    <h3>Ending your use</h3>
    <p>Stop using Atom whenever you want. Clear your browser storage to remove everything. No accounts to close, no cancellation flow.</p>

    <h3>Governing law</h3>
    <p>These terms are governed by the laws of the state where Atom Edu is registered. Disputes go there.</p>

    <h3>Contact</h3>
    <p>hello@atom-edu.org.</p>
  </div>
</section>
...
<div class="modal-veil" id="modal">
  <div class="modal">
    ...
    <p>Verified by a school or district email (.edu or .org).</p>
    <label>Your name</label>
    <input id="in-name" placeholder="Ms. Chen" autocomplete="off">
...
<div class="modal-veil" id="ai-modal">
  <div class="modal modal-lg">
    <div class="ai-head">
      <h3>Your <span class="it">draft.</span></h3>
    ...
    <div class="ai-foot">
      ...
      <button class="ai-use" id="ai-use">Use this draft</button>
    </div>
  </div>
</div>
'''
...
<script>
  ...
  (function(){
    ...
    document.getElementById('toast-undo').addEventListener('click', function(){
      if(lastApprovedRow) lastApprovedRow.classList.remove('gone');
      toast.classList.remove('on');
      showToast('Pulled back to draft.');
    });

    // Approve buttons (queue rows)
    document.querySelectorAll('.qrow .approve').forEach(function(b){
      b.addEventListener('click', function(){
        var row = b.closest('.qrow');
        if(row){ row.classList.add('gone'); lastApprovedRow = row; }
        showToast('Approved.');
      });
    });
    // Approve buttons (approve-bar on maker pages)
    ...
    // Return / Cancel buttons on approve-bar
    ...
    function setAILoading(msg){
      ...
      aiBody.textContent = '';
      aiUse.disabled = true; aiCopy.disabled = true; aiRegen.disabled = true;
    }
    function setAIDone(content){
      aiState.classList.add('hidden');
      aiBody.textContent = content || '';
      aiUse.disabled = false; aiCopy.disabled = false; aiRegen.disabled = false;
    }
    function setAIError(msg){
      ...
      aiUse.disabled = true; aiCopy.disabled = true; aiRegen.disabled = false;
    }

    function systemPromptFor(page){
      var teacherCls = document.getElementById('ws-class-name').textContent || 'a class';
      var base = 'You are Atom, a drafting assistant for teachers. Draft a first version the teacher will edit. Be clear, warm, plain, no purple prose. Never claim to have final authority. The teacher is teaching: ' + teacherCls + '. ';
      var by = {
        assignment: 'Draft a printable worksheet. Include a title, numbered questions with point values in parentheses, and a mix of multiple choice and short answer as requested. Format as plain text ready for the teacher to paste in.',
        ...
        rubric: 'Draft a rubric as a table (Row / 1 Beginning / 2 Developing / 3 Meeting / 4 Exceeding). Tie each row to a standard if given.',
    ...
    aiCopy.addEventListener('click', function(){
      try { navigator.clipboard.writeText(aiBody.textContent || ''); showToast('Copied to clipboard.'); } catch(e){ showToast('Copy failed.'); }
    });
    ...
    // Queue "Open" buttons → navigate to relevant maker page based on tag
    var tagToPage = {
    ...
    document.querySelectorAll('.qrow').forEach(function(row){
      ...
      var target = tagEl ? tagToPage[tagEl.textContent.trim()] : null;
      var openBtn = row.querySelector('.qa button:not(.approve)');
      if(openBtn && target){
    ...
    // Recent rows → open assignment maker as default
    document.querySelectorAll('.rrow').forEach(function(r){
      r.style.cursor = 'pointer';
      r.addEventListener('click', function(){
        var name = (r.querySelector('.name') && r.querySelector('.name').textContent || '').toLowerCase();
        var target = 'assignment';
        if(name.indexOf('rubric') > -1) target = 'rubric';
        else if(name.indexOf('quiz') > -1) target = 'quiz';
        else if(name.indexOf('warm up') > -1 || name.indexOf('lesson') > -1) target = 'lesson';
        else if(name.indexOf('note') > -1 || name.indexOf('message') > -1) target = 'notes';
        else if(name.indexOf('reading') > -1 || name.indexOf('passage') > -1) target = 'passages';
        else if(name.indexOf('slide') > -1) target = 'slides';
        else if(name.indexOf('doc') > -1 || name.indexOf('handout') > -1 || name.indexOf('letter') > -1) target = 'docs';
        else if(name.indexOf('study guide') > -1) target = 'docs';
        goto(target);
      });
    });
